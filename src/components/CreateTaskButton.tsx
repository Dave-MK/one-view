...
export function CreateTaskButton() {
  ...
  function submit(e: React.FormEvent) {
    ...
    logAccess('updated_record', `Logged coordination action: ${t}`)
    setTitle('')
    setOpen(false)
  }
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((o) => !o)} className="inline-flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-white text-sm font-semibold" style={{ backgroundColor: 'var(--brand-800)' }} aria-expanded={open}>
        <Icon name="check" size={15} /> New action
      </button>
      {open && (
        <>
          <button className="fixed inset-0 z-20 cursor-default" aria-label="Close" onClick={() => setOpen(false)} />
          <form onSubmit={submit} className="absolute right-0 mt-2 z-30 w-80 rounded-xl border bg-white p-4 flex flex-col gap-3" style={{ borderColor: 'var(--border)', boxShadow: '0 12px 32px rgba(15,23,42,0.16)' }}>
            <p className="text-sm font-semibold" style={{ color: 'var(--brand-800)' }}>New coordination action — {activeServiceUser.name}</p>
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>A cross-agency action: who owes what, by when. The resulting record stays in the owning service’s own system.</p>
            <input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder="What needs to happen?" className="rounded-lg border px-3 py-2 text-sm" style={{ borderColor: 'var(--border-2)' }} />
            ...
            <button type="submit" disabled={!title.trim()} className="rounded-lg py-2 text-white text-sm font-semibold" style={{ backgroundColor: 'var(--brand-800)', opacity: title.trim() ? 1 : 0.5 }}>Log action</button>
          </form>
        </>
      )}
    </div>
